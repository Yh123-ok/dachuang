...
class MicrostateDGFLayer(nn.Module):
    """引入微状态‘准稳态’(Quasi-stable)特性的动态图融合网络"""
...
class Microstate_MT_GNN_DGF(nn.Module):
    def __init__(self, hidden_dim=64):
        ...
        # --- F. 面部视觉分支 (将512维映射为4个图节点) ---
        # ⚠️注意：如果队友提取的特征不是 512 维，请修改这里的 512
        ...
        # --- D. 全局 DGF 跨模态融合 ---
        # 总节点数: 5(脑区) + 8(微状态) + 8(外周) + 4(视觉) = 25节点
...
# ==========================================
# 4. 高速内存数据加载器 (【修改点】完整替换为支持分 Trial 的版本)
# ==========================================
class DeapLoaderRAM(Dataset):
    def __init__(self, npz_path, mat_path, visual_npy_path, files, vis_dim=512):
        ...
        for f in files:
            ...
            # 3. 【核心修改】逐 Trial 读取队友提取的视觉特征
            subj_visuals = []
            for t_idx in range(1, 41):
                # 寻找形如 s01_trial01_features.npy 的文件
                v_name = f"{sid}_trial{t_idx:02d}_features.npy"
                v_file = os.path.join(visual_npy_path, v_name)
                
                if os.path.exists(v_file):
                    trial_feat = np.load(v_file)
                    
                    # 维度对齐：确保这个 trial 最终映射为 15 个 segments
                    if trial_feat.ndim == 1:
                        trial_feat = np.tile(trial_feat, (15, 1))
                    elif trial_feat.shape[0] != 15:
                        # 如果帧数不是 15，均分为 15 份取平均
                        splits = np.array_split(trial_feat, 15, axis=0)
                        trial_feat = np.vstack([np.mean(seg, axis=0) for seg in splits])
                    
                    subj_visuals.append(trial_feat)
                else:
                    # 容错：缺视频时填充全 0
                    subj_visuals.append(np.zeros((15, self.vis_dim)))
            
            # 拼接 40 个 trial 为 (600, vis_dim)
            vis_list.append(np.concatenate(subj_visuals, axis=0))
...
def run_loso_evaluation():
    ...
    for i, test_f in enumerate(all_files):
        ...
        # 【注意】这里传入了 vis_dim=512，与模型里的 self.vis_node_gen 对应
        train_ds = DeapLoaderRAM(NPZ_DIR, RAW_DIR, VISUAL_DIR, [f for f in all_files if f != test_f], vis_dim=512)
        test_ds = DeapLoaderRAM(NPZ_DIR, RAW_DIR, VISUAL_DIR, [test_f], vis_dim=512)
