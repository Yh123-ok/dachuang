...
# 0. 全局加速设置 (新增)
# ===============================
# 开启 cuDNN 自动调优，加速卷积运算 (针对 ms_enc 中的 Conv2d)
torch.backends.cudnn.benchmark = True
# 允许在 Ampere 及以上架构 GPU 使用 TF32 加速矩阵乘法，精度损失极小但速度翻倍
...
class AdaptiveGNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 【加速核心 1】使用 PyTorch 2.0+ 的 SDPA (融合算子/FlashAttention) 代替手动矩阵相乘
        # 这在不改变数学结果的情况下，大幅降低了显存读写负担，速度提升显著
...
def train_rigorous():
    ...
    for fold, (train_subs, test_subs) in enumerate(kf.split(subjects)):
        ...
        # pin_memory=True 与后面的 non_blocking=True 配合使用
        ...
        # 【加速核心 2】PyTorch 2.0 计算图编译 (如果你的环境是 Windows 且报错，可以注释掉这三行)
        ...
        for ep in range(30):
            ...
            for m, s, p, v, lv, la, ld in train_loader:
                # 【加速核心 3】非阻塞数据传输：让 CPU 传数据和 GPU 计算并行
                ...
                # 【加速核心 4】使用 set_to_none=True 代替默认的 zero_grad，降低显存分配开销
        ...
        with torch.no_grad():
            for m, s, p, v, lv, la, _ in test_loader:
                # 同样应用非阻塞传输，且减少不必要的多次 .to(DEVICE) 调用
                ...
                with torch.autocast(device_type='cuda'): # 推理时也使用 AMP 加速
